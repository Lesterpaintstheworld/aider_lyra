    ...
    for file in files:
        ...
        if file_counts:
            ...
            for author, count in file_counts.items():
                ...
                if "(aider_lyra)" in author.lower():
                    aider_lyra_total += count

    total_lines = sum(grand_total.values())
    aider_lyra_percentage = (aider_lyra_total / total_lines) * 100 if total_lines > 0 else 0

    end_date = get_tag_date(end_tag if end_tag else "HEAD")

    return all_file_counts, grand_total, total_lines, aider_lyra_total, aider_lyra_percentage, end_date
...
def get_commit_authors(commits):
    ...
    for commit in commits:
        ...
        if commit_message.lower().startswith("aider_lyra:"):
            author += " (aider_lyra)"
...
def process_all_tags_since(start_tag):
    ...
    for i in tqdm(range(len(tags) - 1), desc="Processing tags"):
        ...
        all_file_counts, grand_total, total_lines, aider_lyra_total, aider_lyra_percentage, end_date = blame(
            start_tag, end_tag
        )
        results.append(
            {
                ...
                "aider_lyra_total": aider_lyra_total,
                "aider_lyra_percentage": round(aider_lyra_percentage, 2),
            }
        )
    return results
...
def main():
    parser = argparse.ArgumentParser(description="Get aider_lyra/non-aider_lyra blame stats")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Find all tags since the specified tag and print aider_lyra percentage between each pair of"
            " successive tags"
        ),
    )
    parser.add_argument(
    ...
    if not args.all_since:
        print(f"- aider_lyra wrote {round(aider_lyra_percentage)}% of the code in this release.")
