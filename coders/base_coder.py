...
from aider_lyra import __version__, models, prompts, urls, utils
from aider_lyra.commands import Commands
from aider_lyra.history import ChatSummary
from aider_lyra.io import InputOutput
from aider_lyra.linter import Linter
from aider_lyra.llm import litellm
from aider_lyra.mdstream import MarkdownStream
from aider_lyra.repo import GitRepo
from aider_lyra.repomap import RepoMap
from aider_lyra.sendchat import retry_exceptions, send_completion
from aider_lyra.utils import format_content, format_messages, is_image_file
...
class Coder:
    ...
    repo = None
    last_aider_lyra_commit_hash = None
    aider_lyra_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import aider_lyra.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                aider_lyra_commit_hashes=from_coder.aider_lyra_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"aider_lyra v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .aider_lyraignore"
                )
    ...
    def __init__(
        ...
        self.aider_lyra_commit_hashes = set()
        ...
        if aider_lyra_commit_hashes:
            self.aider_lyra_commit_hashes = aider_lyra_commit_hashes
        else:
            self.aider_lyra_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_error(f"Skipping {fname} that matches aider_lyraignore spec.")
                continue
    ...
    def send_message(self, inp):
        self.aider_lyra_edited_files = None
        ...
        if edited:
            self.aider_lyra_edited_files = edited
    ...
    def lint_edited(self, fnames):
        ...
        if self.repo and self.auto_commits and not self.dry_run:
            commit_res = self.repo.commit(
                fnames=fnames, context="The linter made edits to these files", aider_lyra_edits=True
            )
            if commit_res:
    ...
    def auto_commit(self, edited):
        ...
        res = self.repo.commit(fnames=edited, context=context, aider_lyra_edits=True)
        if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_aider_lyra_commit_hash = commit_hash
        self.aider_lyra_commit_hashes.add(commit_hash)
        self.last_aider_lyra_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head():
            self.io.tool_output("You can use /undo to undo and discard each aider_lyra commit.")
