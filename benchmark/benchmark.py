...
def main(
    ...
    replay: str = typer.Option(
        None,
        "--replay",
        help="Replay previous .aider_lyra.chat.history.md responses from previous benchmark run",
    ),
    ...
    no_aider_lyra: bool = typer.Option(False, "--no-aider_lyra", help="Do not run aider_lyra"),
    ...
    if "aider_lyra_DOCKER" not in os.environ:
...
def show_diffs(dirnames):
    ...
    for testcase in testcases:
        ...
        for outcome, dirname in zip(all_outcomes, dirnames):
            print(outcome, f"{dirname}/{testcase}/.aider_lyra.chat.history.md")
...
def load_results(dirname):
    dirname = Path(dirname)
    all_results = [json.loads(fname.read_text()) for fname in dirname.glob("*/.aider_lyra.results.json")]
    return all_results
...
def summarize_results(dirname):
    ...
    command = f"aider_lyra --model {a_model}"
    ...
    for hsh in commit_hashes:
        ...
        try:
            version = subprocess.check_output(
                ["git", "show", f"{hsh}:aider_lyra/__init__.py"], universal_newlines=True
            )
...
def get_replayed_content(replay_dname, test_dname):
    ...
    replay_fname = replay_dname / test_name / ".aider_lyra.chat.history.md"
    dump(replay_fname)
...
def run_test(original_dname, testdir, *args, **kwargs):
    try:
        ...
        results_fname = testdir / ".aider_lyra.results.json"
...
def run_test_real(
    ...
    history_fname = testdir / ".aider_lyra.chat.history.md"
