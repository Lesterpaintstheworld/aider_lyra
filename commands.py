...
from aider_lyra import models, prompts, voice
from aider_lyra.help import Help, install_help_extra
from aider_lyra.llm import litellm
from aider_lyra.scrape import Scraper, install_playwright
from aider_lyra.utils import is_image_file
...
class Commands:
    ...
    def cmd_undo(self, args):
        "Undo the last git commit if it was done by aider_lyra"
        if not self.coder.repo:
            self.io.tool_error("No git repository found.")
            return
        ...
        if last_commit_hash not in self.coder.aider_lyra_commit_hashes:
            self.io.tool_error("The last commit was not made by aider_lyra in this chat session.")
            self.io.tool_error(
    ...
    def basic_help(self):
        ...
        self.io.tool_output()
        self.io.tool_output("Use `/help <question>` to ask questions about how to use aider_lyra.")

    def cmd_help(self, args):
        "Ask questions about aider_lyra"
        ...
        user_msg += """
# Announcement lines from when this session of aider_lyra was launched:
