...
import aider_lyra
from aider_lyra import utils
from aider_lyra.dump import dump  # noqa: F401


def check_version(io, just_check=False, verbose=False):
    fname = Path.home() / ".aider_lyra" / "caches" / "versioncheck"
    ...
    try:
        response = requests.get("https://pypi.org/pypi/aider_lyra-chat/json")
        data = response.json()
        latest_version = data["info"]["version"]
        current_version = aider_lyra.__version__
    ...
    docker_image = os.environ.get("aider_lyra_DOCKER_IMAGE")
    if docker_image:
        text = f"""
Newer aider_lyra version v{latest_version} is available. To upgrade, run:
...
"""
    ...
    cmd = utils.get_pip_install(["--upgrade", "aider_lyra-chat"])
...
"""
    ...
    if io.confirm_ask("Run pip install?"):
        ...
        if success:
            io.tool_output("Re-run aider_lyra to use new version.")
            sys.exit()
        else:
