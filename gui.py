...
def search(text=None):
    results = []
    for root, _, files in os.walk("aider_lyra"):
        for file in files:
...
class GUI:
    ...
    def show_edit_info(self, edit):
        ...
        if commit_hash:
            ...
            if commit_hash == self.coder.last_aider_lyra_commit_hash:
                show_undo = True
    ...
    def do_sidebar(self):
        with st.sidebar:
            ...
            st.warning(
                "This browser version of aider_lyra is experimental. Please share feedback in [GitHub"
                " issues](https://github.com/paul-gauthier/aider_lyra/issues)."
            )
    ...
    def do_recommended_actions(self):
        text = "aider_lyra works best when your code is stored in a git repo.  \n"
        ...
        with st.expander("Recommended actions", expanded=True):
            ...
            with st.popover("Update your `.gitignore` file"):
                st.write("It's best to keep aider_lyra's internal files out of your git repo.")
                self.button("Add `.aider_lyra*` to `.gitignore`", key=random.random(), help="?")
    ...
    def initialize_state(self):
        ...
        self.state.init("last_aider_lyra_commit_hash", self.coder.last_aider_lyra_commit_hash)
    ...
    def process_chat(self):
        ...
        with self.messages:
            edit = dict(
                role="edit",
                fnames=self.coder.aider_lyra_edited_files,
            )
            if self.state.last_aider_lyra_commit_hash != self.coder.last_aider_lyra_commit_hash:
                edit["commit_hash"] = self.coder.last_aider_lyra_commit_hash
                edit["commit_message"] = self.coder.last_aider_lyra_commit_message
                commits = f"{self.coder.last_aider_lyra_commit_hash}~1"
                diff = self.coder.repo.diff_commits(
                    self.coder.pretty,
                    commits,
                    self.coder.last_aider_lyra_commit_hash,
                )
                edit["diff"] = diff
                self.state.last_aider_lyra_commit_hash = self.coder.last_aider_lyra_commit_hash
    ...
    def do_undo(self, commit_hash):
        ...
        if (
            self.state.last_aider_lyra_commit_hash != commit_hash
            or self.coder.last_aider_lyra_commit_hash != commit_hash
        ):
...
def gui_main():
    st.set_page_config(
        layout="wide",
        page_title="aider_lyra",
        page_icon=urls.favicon,
        menu_items={
            "Get Help": urls.website,
            "Report a bug": "https://github.com/paul-gauthier/aider_lyra/issues",
