...
from aider_lyra.utils import GitTemporaryDirectory
...
class TestRepo(unittest.TestCase):
    ...
    @patch("aider_lyra.repo.simple_send_with_retries")
    ...
    @patch("aider_lyra.repo.GitRepo.get_commit_message")
    def test_commit_with_custom_committer_name(self, mock_send):
        ...
        with GitTemporaryDirectory():
            ...
            git_repo.commit(fnames=[str(fname)], aider_lyra_edits=True)

            # check the committer name
            commit = raw_repo.head.commit
            self.assertEqual(commit.author.name, "Test User (aider_lyra)")
            self.assertEqual(commit.committer.name, "Test User (aider_lyra)")

            # commit a change without aider_lyra_edits
            fname.write_text("new content again!")
            git_repo.commit(fnames=[str(fname)], aider_lyra_edits=False)
    ...
    def test_get_tracked_files_with_aider_lyraignore(self):
        with GitTemporaryDirectory():
            # new repo
            raw_repo = git.Repo()
            ...
            aider_lyraignore = Path(".aider_lyraignore")
            git_repo = GitRepo(InputOutput(), None, None, str(aider_lyraignore))
            ...
            aider_lyraignore.write_text("new.txt\n")
            time.sleep(2)
            ...
            #
            # aider_lyraignore.write_text("new2.txt\n")
