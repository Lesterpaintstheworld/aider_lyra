...
from aider_lyra.main import check_gitignore, main, setup_git
from aider_lyra.utils import GitTemporaryDirectory, IgnorantTemporaryDirectory, make_repo
...
class TestMain(TestCase):
    ...
    @patch("aider_lyra.repo.GitRepo.get_commit_message", return_value="mock commit message")
    ...
    def test_main_with_git_config_yml(self):
        ...
        Path(".aider_lyra.conf.yml").write_text("auto-commits: false\n")
        with patch("aider_lyra.coders.Coder.create") as MockCoder:
            main(["--yes"], input=DummyInput(), output=DummyOutput())
            _, kwargs = MockCoder.call_args
            assert kwargs["auto_commits"] is False

        Path(".aider_lyra.conf.yml").write_text("auto-commits: true\n")
        with patch("aider_lyra.coders.Coder.create") as MockCoder:
    ...
    def test_main_with_empty_git_dir_new_subdir_file(self):
        ...
        # Because aider_lyra will try and `git add` a file that's already in the repo.
    ...
    def test_setup_git(self):
        ...
        self.assertEqual(".aider_lyra*", gitignore.read_text().splitlines()[0])
    ...
    def test_encodings_arg(self):
        ...
        with GitTemporaryDirectory():
            with patch("aider_lyra.coders.Coder.create") as MockCoder:  # noqa: F841
                with patch("aider_lyra.main.InputOutput") as MockSend:
    ...
    def test_main_exit_calls_version_check(self):
        with GitTemporaryDirectory():
            with (
                patch("aider_lyra.main.check_version") as mock_check_version,
                patch("aider_lyra.main.InputOutput") as mock_input_output,
            ):
                main(["--exit"], input=DummyInput(), output=DummyOutput())
                mock_check_version.assert_called_once()
                mock_input_output.assert_called_once()

    @patch("aider_lyra.main.InputOutput")
    @patch("aider_lyra.coders.base_coder.Coder.run")
    ...
    def test_env_file_flag_sets_automatic_variable(self):
        env_file_path = self.create_env_file(".env.test", "aider_lyra_DARK_MODE=True")
        with patch("aider_lyra.coders.Coder.create") as MockCoder:
            main(
    ...
    def test_default_env_file_sets_automatic_variable(self):
        self.create_env_file(".env", "aider_lyra_DARK_MODE=True")
        with patch("aider_lyra.coders.Coder.create") as MockCoder:
            main(["--no-git"], input=DummyInput(), output=DummyOutput())
            # Ensure Coder.create was called
            MockCoder.assert_called_once()
    ...
    def test_false_vals_in_env_file(self):
        self.create_env_file(".env", "aider_lyra_SHOW_DIFFS=off")
        with patch("aider_lyra.coders.Coder.create") as MockCoder:
            main(["--no-git"], input=DummyInput(), output=DummyOutput())
            MockCoder.assert_called_once()
            _, kwargs = MockCoder.call_args
    ...
    def test_true_vals_in_env_file(self):
        self.create_env_file(".env", "aider_lyra_SHOW_DIFFS=on")
        with patch("aider_lyra.coders.Coder.create") as MockCoder:
            main(["--no-git"], input=DummyInput(), output=DummyOutput())
            MockCoder.assert_called_once()
            _, kwargs = MockCoder.call_args
    ...
    def test_lint_option(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            with patch("aider_lyra.linter.Linter.lint") as MockLinter:
    ...
    def test_verbose_mode_lists_env_vars(self):
        self.create_env_file(".env", "aider_lyra_DARK_MODE=on")
        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            ...
            relevant_output = "\n".join(
                ...
                if "aider_lyra_DARK_MODE" in line or "dark_mode" in line
            )  # this bit just helps failing assertions to be easier to read
            self.assertIn("aider_lyra_DARK_MODE", relevant_output)
            self.assertIn("dark_mode", relevant_output)
            self.assertRegex(relevant_output, r"aider_lyra_DARK_MODE:\s+on")
    ...
    def test_yaml_config_file_loading(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            # Create .aider_lyra.conf.yml files in different locations
            home_config = fake_home / ".aider_lyra.conf.yml"
            git_config = git_dir / ".aider_lyra.conf.yml"
            cwd_config = cwd / ".aider_lyra.conf.yml"
            named_config = git_dir / "named.aider_lyra.conf.yml"
            ...
            with (
                ...
                patch("aider_lyra.coders.Coder.create") as MockCoder,
            ):
    ...
    def test_map_tokens_option(self):
        with GitTemporaryDirectory():
            with patch("aider_lyra.coders.base_coder.RepoMap") as MockRepoMap:
    ...
    def test_model_metadata_file(self):
        with GitTemporaryDirectory():
            metadata_file = Path(".aider_lyra.model.metadata.json")
