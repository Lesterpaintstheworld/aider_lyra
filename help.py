...
from aider_lyra import __version__, utils
from aider_lyra.dump import dump  # noqa: F401
from aider_lyra.help_pats import exclude_website_pats
...
def get_package_files():
    for path in importlib_resources.files("aider_lyra.website").iterdir():
        if path.is_file():
            yield path
        elif path.is_dir():
...
def fname_to_url(filepath):
    ...
    if filepath.startswith("website/_includes/"):
        ...
        docid = "https://aider_lyra.chat/" + filepath
...
def get_index():
    ...
    dname = Path.home() / ".aider_lyra" / "caches" / ("help." + __version__)
    ...
    if dname.exists():
        ...
        for fname in get_package_files():
            ...
            doc = Document(
                text=importlib_resources.files("aider_lyra.website")
                .joinpath(fname)
