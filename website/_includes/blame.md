...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var blameData = {
        labels: labels,
        datasets: [{
            label: 'aider_lyra\'s percent of new code by release',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.aider_lyra_percentage }}, lines: {{ row.aider_lyra_total }} },{% endfor %}],
    ...
    var linesData = {
        labels: labels,
        datasets: [{
            label: 'aider_lyra\'s lines of new code',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.aider_lyra_total }} },{% endfor %}],
    ...
    var blameChart = new Chart(blameCtx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            var label = 'aider_lyra\'s contribution';
                ...
                title: {
                    display: true,
                    text: 'Percent of new code written by aider_lyra, by release',
                    font: {
                        size: 16
                    }
                }
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            var label = 'New lines of code by aider_lyra';
                            var value = context.parsed.y || 0;
                            return `${label}: ${value}`;
                        }
                    }
                ...
                title: {
                    display: true,
                    text: 'Lines of new code written by aider_lyra, by release',
                    font: {
                        size: 16
                    }
                }
