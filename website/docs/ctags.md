...
aider_lyra no longer uses ctags to build a repo map.
Please see the newer article about
[using tree-sitter to build a better repo map](https://aider_lyra.chat/docs/repomap.html).
...
To address these issues, `aider_lyra` now
...
[chat transcript](https://aider_lyra.chat/examples/add-test.html)
...
To code with GPT-4 using the techniques discussed here:
  ...
  - Install [aider_lyra](https://aider_lyra.chat/docs/install.html).
  - Install universal ctags.
  - Run `aider_lyra` inside your repo, and it should say "Repo-map: universal-ctags using 1024 tokens".
...
The latest version of `aider_lyra` sends a **repo map** to GPT along with
...
[main.py](https://github.com/paul-gauthier/aider_lyra/blob/main/aider_lyra/main.py)
and
[io.py](https://github.com/paul-gauthier/aider_lyra/blob/main/aider_lyra/io.py)
:
...
Mapping out the repo like this provides some benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at. GPT will then ask to see these specific files, and `aider_lyra` will automatically add them to the chat context (with user approval).
...
Sending an appropriate subset of the repo map would help `aider_lyra` work
