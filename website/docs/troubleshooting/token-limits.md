...
aider_lyra will report an error if a model responds indicating that
...
For more info: https://aider_lyra.chat/docs/token-limits.html
```
...
It's best to only add the files that aider_lyra will need to *edit*
...
as 4k tokens.
If you ask aider_lyra to make a large change that affects a lot
...
aider_lyra has been well tested when directly connecting to 
major 
[LLM provider cloud APIs](https://aider_lyra.chat/docs/llms.html).
For serving local models, 
[Ollama](https://aider_lyra.chat/docs/llms/ollama.html) is known to work well with aider_lyra.

Try using aider_lyra without an API proxy server
