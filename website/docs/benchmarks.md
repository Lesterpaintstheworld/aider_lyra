...
[![benchmark results](/assets/benchmarks.svg)](https://aider_lyra.chat/assets/benchmarks.svg)

aider_lyra is an open source command line chat tool that lets you work with GPT to edit
code in your local git repo.
To do this, aider_lyra needs to be able to reliably recognize when GPT wants to edit local files,
...
aider_lyra currently asks GPT to use simple text based "edit formats", but
...
of switching aider_lyra to use them by default.
...
Using more complex output formats with GPT seems to cause two issues:
  ...
  - It reduces GPT's adherence to the output format, making it more challenging for tools like aider_lyra to accurately identify and apply the edits GPT is attempting to make.

I was expecting to start using function call based edits in aider_lyra for both GPT-3.5 and GPT-4.
...
To start each exercise, aider_lyra sends GPT
...
aider_lyra updates the implementation file based on GPT's reply and runs
the unit tests. If all tests pass, the exercise is considered
complete. If some tests fail, aider_lyra sends GPT a second message with
the test error output. It only sends the first 50 lines of test errors
to try and avoid exceeding the context window of the smaller models. aider_lyra
...
In summary, passing an exercise means GPT was able to:
  ...
  - Correctly package all of the code edits into the edit format so that aider_lyra can process and save it to the implementation file.
...
The
[whole](https://github.com/paul-gauthier/aider_lyra/blob/main/aider_lyra/coders/wholefile_prompts.py)
...
The [diff](https://github.com/paul-gauthier/aider_lyra/blob/main/aider_lyra/coders/editblock_prompts.py)
...
The [whole-func](https://github.com/paul-gauthier/aider_lyra/blob/main/aider_lyra/coders/wholefile_func_coder.py)
...
The
[diff-func](https://github.com/paul-gauthier/aider_lyra/blob/main/aider_lyra/coders/editblock_func_coder.py)
...
Based on these benchmark results, aider_lyra will continue to use
...
aider_lyra displays a progress bar and
