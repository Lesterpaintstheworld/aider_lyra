...
[![benchmark results](/assets/benchmarks-1106.svg)](https://aider_lyra.chat/assets/benchmarks-1106.svg)

[![benchmark results](/assets/benchmarks-speed-1106.svg)](https://aider_lyra.chat/assets/benchmarks-speed-1106.svg)
...
To do this, aider_lyra needs to be able to reliably recognize when GPT wants to edit
your source code,
...
For example,
whenever I change aider_lyra's prompting or the backend which drives LLM conversations,
...
The benchmark gives aider_lyra two tries to complete the task:

1. On the first try, aider_lyra gives GPT the stub code file to edit and the natural language instructions that describe the problem. This reflects how you code with aider_lyra. You add your source code files to the chat and ask for changes, which are automatically applied.
2. If the test suite fails after the first try, aider_lyra gives GPT the test error output and asks it to fix the code. aider_lyra supports this sort of interaction using a command like `/run pytest` to run and share pytest results in the chat with GPT. You can `/run` whatever tests/linters/etc make sense for your language/framework/situation.
...
This is the edit format that aider_lyra uses by default with gpt-4.
...
- [Code editing speed benchmarks for OpenAI's "1106" models](https://aider_lyra.chat/2023/11/06/benchmarks-speed-1106.html) compares the performance of the new GPT models.
