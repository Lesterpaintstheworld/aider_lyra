...
parent: Usage
nav_order: 50
description: Control aider_lyra with in-chat commands like /add, /model, etc.
---
# In-chat commands

aider_lyra supports commands from within the chat, which all start with `/`.

<!--[[[cog
from aider_lyra.commands import get_help_md
cog.out(get_help_md())
]]]-->
...
| **/help** | Ask questions about aider_lyra |
...
| **/undo** | Undo the last git commit if it was done by aider_lyra |
...
To use vi/vim keybindings, run aider_lyra with the `--vim` switch.
