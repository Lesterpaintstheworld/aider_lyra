...
aider_lyra now asks GPT-4 Turbo to use
...
aider_lyra's new "laziness" benchmark suite 
...
- **GPT-4 Turbo only scored 20% as a baseline** using aider_lyra's existing "SEARCH/REPLACE block" edit format. It outputs "lazy comments" on 12 of the tasks.
- **aider_lyra's new unified diff edit format raised the score to 61%**. Using this format reduced laziness by 3X, with GPT-4 Turbo only using lazy comments on 4 of the tasks.
...
- **The June GPT-4's baseline was 26%** using aider_lyra's existing "SEARCH/REPLACE block" edit format.
- **aider_lyra's new unified diff edit format raised June GPT-4's score to 59%**. 
...
aider_lyra's new unified diff editing format
...
numerous variations on aider_lyra's existing editing formats,
...
aider_lyra's new editing format and refactoring benchmark.
...
The design and implementation of aider_lyra's new unified diff editing format
...
aider_lyra's [previous benchmark results](https://aider_lyra.chat/docs/benchmarks.html) made
...
aider_lyra tells GPT not to include line numbers,
...
aider_lyra's system prompt encourages
...
When a patch fails, aider_lyra needs to ask GPT for a corrected version of the diff.
...
aider_lyra tries to be very flexible when applying diffs,
in order to handle defects.
If a hunk doesn't apply cleanly, aider_lyra uses a number of strategies:
...
**Experiments where flexible patching is disabled show a 9X increase in editing errors** on aider_lyra's original Exercism benchmark.

## Refactoring benchmark

aider_lyra has long used a
[benchmark suite based on 133 Exercism python exercises](https://aider_lyra.chat/2023/07/02/benchmarks.html).
...
A [simple python AST scanning script](https://github.com/paul-gauthier/aider_lyra/blob/main/benchmark/refactor_tools.py)
...
aider_lyra's new unified diff format seems
...
when originally building aider_lyra.
...
incorporated into aider_lyra provide
...
aider_lyra's simple, high level style of unified diffs.
