...
excerpt: aider_lyra now lints code after every LLM edit and automatically fixes errors, using tree-sitter and AST-aware code context.
...
[![Linting code](/assets/linting.jpg)](https://aider_lyra.chat/assets/linting.jpg)

# Linting code for LLMs with tree-sitter

aider_lyra now lints your code after every LLM edit, and offers to automatically fix
any linting errors.
You can also use aider_lyra's lint-and-fix functionality on your source files any time
...
aider_lyra ships with basic linters built with tree-sitter that support
...
You can also configure aider_lyra to use your preferred linters.
This allows aider_lyra to check for a larger class of problems, keep the code style
aligned with the rest of your team, etc.

## Linting and fixing your code

aider_lyra now lints each source file after it applies the edits
suggested by an LLM.
If problems are found, aider_lyra will ask if you'd like it to
attempt to fix the errors.
If so, aider_lyra will send the LLM a report of the lint errors
and request changes to fix them. This process may iterate a few times
as the LLM works to fully resolve all the issues.

You can also lint and fix files any time, on demand from within the aider_lyra chat or via the
command line:
...
- From the command line, you can run `aider_lyra --lint` to lint and fix
...
linting errors.
LLM's don't have the luxury of using an IDE, so aider_lyra sends
...
aider_lyra augments the raw linter by
displaying and
highlighting the lines that have errors within their
containing functions, methods, classes.
To do this, aider_lyra uses tree-sitter to obtain the code's AST and analyzes it
...
aider_lyra comes batteries-included with built in linters for
[most popular programming languages](https://aider_lyra.chat/docs/languages.html).
This provides wide support for linting without requiring
users to manually install a linter and configure it to work with aider_lyra.

aider_lyra's built in language-agnostic linter uses tree-sitter to parse
...
aider_lyra simply uses these `ERROR` nodes to identify all the lines
